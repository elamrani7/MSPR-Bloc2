...
pour le chiffrement du secret TOTP avant stockage en base, On va utiliser cryptography.fernet
Très important : la même clé doit être utilisée dans generate-2fa et authenticate-user. Sinon, l’authentification ne pourra pas déchiffrer le secret. $env:TOTP_ENCRYPTION_KEY="COLLE_TA_CLE_ICI"
- commanse pour accéder à notre BD postgre : docker exec -it cofrap-postgres psql -U cofrap -d cofrapdb
- commande pour Vérifier que le secret est bien chiffré en base