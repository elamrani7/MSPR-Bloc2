...
def decrypt_totp_secret(encrypted_secret):
    ...
    decrypted_secret = fernet.decrypt(encrypted_secret.encode("utf-8"))
...
def verify_totp_code(otp_code, encrypted_totp_secret):
    if not encrypted_totp_secret:
        return False

    try:
        totp_secret = decrypt_totp_secret(encrypted_totp_secret)
    except Exception:
        return False
